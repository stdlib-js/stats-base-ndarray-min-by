...
tape( 'the function has an arity of 3', function test( t ) {
	t.strictEqual( minBy.length, 3, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the minimum value of a one-dimensional ndarray via a callback function', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ];
	v = minBy( [ vector( x, 6, 1, 0 ) ], clbk );
	t.strictEqual( v, -8.0, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = minBy( [ vector( x, 2, 1, 0 ) ], clbk );
	t.strictEqual( v, -10.0, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = minBy( [ vector( x, 3, 1, 0 ) ], clbk );
	t.strictEqual( isNegativeZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = minBy( [ vector( x, 1, 1, 0 ) ], clbk );
